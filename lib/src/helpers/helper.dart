    ...
    if (currentDayStatus == false) {
      if (openTime[1] == 'AM' && closeTime[1] == 'AM') {
        startTime = DateTime(now.year, now.month, now.day,
            int.parse(openHrm[0].trim()), int.parse(openHrm[1].trim()));
        endTime = DateTime(now.year, now.month, now.day,
            int.parse(closeHrm[0].trim()), int.parse(closeHrm[1].trim()));
      } else if (openTime[1] == 'AM' && closeTime[1] == 'PM') {
        startTime = DateTime(now.year, now.month, now.day,
            int.parse(openHrm[0].trim()), int.parse(openHrm[1].trim()));
        endTime = DateTime(now.year, now.month, now.day,
            int.parse(closeHrm[0].trim()) + 12, int.parse(closeHrm[1].trim()));
      } else if (openTime[1] == 'PM' && closeTime[1] == 'PM') {
        startTime = DateTime(now.year, now.month, now.day,
            int.parse(openHrm[0].trim()) + 12, int.parse(openHrm[1].trim()));
        endTime = DateTime(now.year, now.month, now.day,
            int.parse(closeHrm[0].trim()) + 12, int.parse(closeHrm[1].trim()));
      }
