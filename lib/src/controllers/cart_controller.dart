...
import 'package:edeli/src/components/directions_model.dart';
import 'package:edeli/src/components/directions_repository.dart';
import 'package:edeli/src/models/recharge.dart';
import 'package:edeli/src/repository/home_repository.dart';
import 'package:edeli/src/repository/settings_repository.dart';
import 'package:edeli/src/repository/wallet_repository.dart';
...
class CartController extends ControllerMVC {
  ...
  getOriginalDistance() async {
    ...
    final directions = await DirectionsRepository()
        .getDirections(origin: _origin, destination: _destination);
  ...
  grandSummary() {
    ...
    tax =
        double.parse(tax.toStringAsFixed(setting.value.currencyDecimalDigits));

    currentCheckout.value.packingCharge = packingCharge;

    saveamount = double.parse(totalsalesprice
            .toStringAsFixed(setting.value.currencyDecimalDigits)) -
        double.parse(
            totalprice.toStringAsFixed(setting.value.currencyDecimalDigits));
    currentCheckout.value.sub_total = double.parse((totalprice + addon)
        .toStringAsFixed(setting.value.currencyDecimalDigits));
    ...
    if (currentCheckout.value.couponStatus == false) {
      if (currentCheckout.value.deliverType != 3) {
        currentCheckout.value.grand_total = double.parse((totalprice +
                addon +
  ...
  applyCoupon(CouponModel couponData, type) {
    ...
    saveamount = double.parse(
            grandTotal.toStringAsFixed(setting.value.currencyDecimalDigits)) -
        double.parse(totalstrikeprice
            .toStringAsFixed(setting.value.currencyDecimalDigits));

    if (grandTotal >= couponData.minimumPurchasedAmount) {
      if (couponData.couponType == '1' ||
          couponData.couponType == '2' ||
          couponData.couponType == '3') {
        if (couponData.discountType == 'amount') {
          discountAmount = saveamount + couponData.discount;
          currentCheckout.value.discount = double.parse(discountAmount
                  .toStringAsFixed(setting.value.currencyDecimalDigits))
              .abs();
          currentCheckout.value.grand_total =
              (grandTotal - couponData.discount) +
                  packingCharge +
                  tax +
                  currentCheckout.value.delivery_fees;
        } else {
          ...
          currentCheckout.value.discount = double.parse(currentCheckout
                  .value.discount
                  .toStringAsFixed(setting.value.currencyDecimalDigits))
              .abs();
          currentCheckout.value.grand_total = (grandTotal - discountAmount) +
              packingCharge +
              tax +
              currentCheckout.value.delivery_fees;
        }
      ...
      if (type == 'direct') {
        showToast(S.of(context).your_coupon_is_applied_successfully,
  ...
  removeCoupon() {
    ...
    currentCheckout.value.payment.tax = tax;
    currentCheckout.value.discount = saveamount + discountAmount;
    currentCheckout.value.discount = double.parse(currentCheckout.value.discount
            .toStringAsFixed(setting.value.currencyDecimalDigits))
        .abs();
    currentCheckout.value.grand_total = (grandTotal - discountAmount) +
        packingCharge +
    ...
    showToast(S.of(context).your_coupon_is_removed_successfully,
        gravity: Toast.BOTTOM, duration: Toast.LENGTH_SHORT);
  }
    ...
    if (currentCheckout.value.payment.method == 'online') {
      ...
      if (currentCheckout.value.payment.paymentType == 'Paypal') {
        Navigator.of(context).push(
            MaterialPageRoute(builder: (context) => PayPalPaymentWidget()));
      }
      if (currentCheckout.value.payment.paymentType == 'Stripe') {
        Navigator.of(context).push(
            MaterialPageRoute(builder: (context) => StripePaymentWidget()));
      }
      if (currentCheckout.value.payment.paymentType == 'flutterwave') {
        Navigator.of(context).push(MaterialPageRoute(
            builder: (context) => FlutterWavePaymentWidget()));
      }
  ...
  paymentProcess() {
    currentCheckout.value.payment.packingCharge =
        currentCheckout.value.packingCharge;
    currentCheckout.value.payment.delivery_tips =
        currentCheckout.value.delivery_tips;
    currentCheckout.value.payment.delivery_fees =
        currentCheckout.value.delivery_fees;
    ...
    currentCheckout.value.payment.grand_total =
        currentCheckout.value.grand_total;
  }
  ...
  void bookOrder(Checkout order) {
    ...
    FirebaseFirestore.instance
      ...
      'processingTime': Helper.calculateTime(
          currentCheckout.value.km, currentCheckout.value.handoverTime, true),
    }).catchError((e) {
      print(e);
    });
    ...
    bookOrderResp().then((value) {
      ...
      if (currentCheckout.value.payment.paymentType == 'wallet') {
        currentUser.value.walletAmount =
            (double.parse(currentUser.value.walletAmount) -
                    currentCheckout.value.payment.grand_total)
                .toString();
  ...
  calculateDistance(lat1, lon1, lat2, lon2) {
    ...
    double distance = Helper.distance(lat1, lon1, double.parse(lat2),
        double.parse(lon2), setting.value.distanceUnit);
    if (1 == 2) {
