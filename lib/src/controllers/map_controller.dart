...
import 'package:edeli/src/models/providerlist.dart';
import 'package:edeli/src/models/shop_type.dart';
import 'package:edeli/src/models/takeaway.dart';
...
class MapController extends ControllerMVC {
  ...
  void listenForNearMarketsWithID(
      Address myLocation, Address areaLocation, id) async {
    setState(() {
      topMarkets.clear();
    });

    final Stream<Vendor> stream =
        await getNearMarkets(myLocation, areaLocation, id);
    stream.listen((Vendor _market) {
      // Helper.calculateDistance();
  ...
  void getCurrentProviderLocation() async {
    try {
      setState(() {
        cameraPosition = CameraPosition(
          target: LatLng(currentBookDetail.value.latitude,
              currentBookDetail.value.longitude),
          zoom: 14.4746,
        );
      });

      Helper.getMyPositionMarker(currentBookDetail.value.latitude,
              currentBookDetail.value.longitude)
          .then((marker) {
        setState(() {
          allMarkers.add(marker);
  ...
  void getProvider() async {
    setState(() {
      ...
      Address areaAddress = Address.fromJSON({
        "latitude": currentBookDetail.value.latitude,
        "longitude": currentBookDetail.value.longitude
      });
  ...
  void listenForNearProvider(Address myLocation, Address areaLocation) async {
    final Stream<ProviderList> stream =
        await getNearProvider(myLocation, areaLocation);
