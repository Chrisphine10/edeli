...
Future<Stream<Vendor>> getNearMarkets(
...
Future<TakeawayModel> getTakeawayDetail(String id) async {
  ...
  final String url =
      '${GlobalConfiguration().getString('api_base_url')}api/takeaway/list/$id?$_apiToken';
  final client = new http.Client();
...
Future<Stream<ProviderList>> getNearProvider(
    Address myLocation, Address areaLocation) async {
  Uri uri = Helper.getUri(
      'api/listProvider/${currentBookDetail.value.categoryId}/${currentBookDetail.value.subcategoryId}');
  Map<String, dynamic> _queryParams = {};
