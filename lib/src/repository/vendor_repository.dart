...
Future<Stream<Vendor>> getVendorListOffer(
    shopType, offer, offerType, shopTypeId) async {
  Uri uri = Helper.getUri(
      'api/shoplistOffer/$shopType/$offer/$offerType/$shopTypeId');
  Map<String, dynamic> _queryParams = {};
...
Future<bool> addFavoriteShop() async {
  ...
  final String url =
      '${GlobalConfiguration().getValue('api_base_url')}api/favoriteShop/do_add/${currentUser.value.id}/?$_apiToken';
...
Future<Stream<ProductDetails2>> getShopProductSlide(id) async {
  ...
  final String url =
      '${GlobalConfiguration().getValue('api_base_url')}api/shopSetting/productList/$id/${currentUser.value.id}?$_apiToken';
